...
**Response**: No, because there are outliers in the data set and it has a skewed distribution
...
***Response**: No because the data is not normally distributed
...
**Response**: 90% confident that mean number of years of education is between 13.65 and 13.83
...
```{r}
set.seed(1234)
edu <- replicate(1000, sample(gss_educ$educ, size=nrow(gss_educ), replace=TRUE), simplify=FALSE)
meanedu <- map_dbl(edu, mean)
```
...
```{r}
quantile(meanedu, probs=c(0.10, 0.90))
```


Compare this bootstrap interval to your `t.test` confidence interval from above.

The t.test and the bootstrap interval are both very close in value. They do not differ much. 
...
set.seed(1234)
funct <- function(){
  df2 <- replicate(1000, sample(gss_educ$educ, size=nrow(gss_educ), replace=TRUE), simplify=FALSE)
  df3 <- tibble(
  mean = map_dbl(df2, mean),
  median = map_dbl(df2, median),
  midhinge = map_dbl(df2, statip::midhinge)
  )
}
df3 <- funct()
```
...
```{r}
quantile(df3$mean, probs=c(0.10, 0.90))
quantile(df3$median, probs=c(0.10, 0.90))
quantile(df3$midhinge, probs=c(0.10, 0.90))
```
